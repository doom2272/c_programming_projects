...
/*MODIFIES poker.c to check for 'royal flush' - where a hand has ace, king, queen, jack and ten of the same suit.
 *
...
void analyze_hand(void)
{
  ...
  if (rank > 8) skip_straight = true;

  for (; rank < NUM_RANKS && num_in_rank[rank] > 0 && !skip_straight; rank++)
    num_consec++;
