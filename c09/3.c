 ...
 * now uses functions to generate the random walk and print the array.
 */
...
void generate_random_walk(char walk[10][10])
{
  ...
  for (int i = 0; i < N; i++) { // adds a period to each element in walk
    for (int j = 0; j < N; j++) {
      walk[i][j] = '.';
    }
  }
  ...
  walk[0][0] = 'A'; //always start path from walk[0][0]
  ...
  for (char ch = 'B'; ch <= 'Z'; ch++) { 
    ...
    while (!tries[0] || !tries[1] || !tries[2] || !tries[3]) { //as long as all possible moves have not been exhausted
      ...
      if (next_r >= 0 && next_r < N && next_c >= 0 && next_c < N && walk[next_r][next_c] == '.') { // as long as our next move is within array boundary and the element there is a period(.)
        r = next_r; // we can safely move to the next row
        c = next_c; // we can safely move to the next column
        walk[r][c] = ch; // then add a letter 
