...
int main(void)
{
  ...
  if (n % 2 == 0 || n < 0 || n > 99){ //ensures user enters an odd number between 1 - 99
    printf("\nSize should be an odd number and range between 1 - 99\n");
    return 1;
  }
...
void create_magic_square(int n, int magic_square[n][n])
{
  int row, col, current_row, current_col;
  ...
  while (num <= n * n) { //as long as we dont exceed our square, which is the largest nimber
    ...
    if (magic_square[row][col] == 0) magic_square[row][col] = num++; //if our next move has a zero populate it with a number, then increment num
    else { // if we already filled the next spot, remain on the same column but move a row down
      if (current_row + 1 >= n) 
        magic_square[row = 0][col = current_col] = num++;
      else 
        magic_square[row = current_row + 1][col = current_col] = num++;
    }
  }
...
void print_magic_square(int n, int magic_square[n][n])
{
  ...
  for (int i = 0; i < n; i++) { //prints our magic square
    for (int j = 0; j < n; j++){
      printf("%*d ", width, magic_square[i][j]); //the asterix uses the width variable to adjust our minimum number of digits to print. so if our largest number is 9, print 1 digit, if its 25, print a minimum of 2 digits, if its 121, print a minimumof 3 digits for each element in our array
    }
    putchar('\n');
  }
}
