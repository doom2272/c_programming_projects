...
// modifies the program by making the print function display the parts sorted by part number.
...
void print(void)
{
  ...
  for (i = 0; i < num_parts - 1; i++)
    for (j = i+1; j < num_parts; j++)
      if (temp_inventory[i].number > temp_inventory[j].number) {

        struct part temp = temp_inventory[i];

        temp_inventory[i] = temp_inventory[j];
        temp_inventory[j] = temp;
      }
  ...
  printf("Part Number   Part Name                  "
      "Quantity on Hand\n");
  for (i = 0; i < num_parts; i++)
    printf("%7d       %-25s%11d\n", temp_inventory[i].number, temp_inventory[i].name, temp_inventory[i].on_hand);
}
